...
class Parser:
    ...
    def if_statement(self):
        ...
        elif self.tokens[self.idx - 2].value == "DO":
            action = self.statement()
            return [condition, action]
    ...
    def if_statements(self):
        conditions = []
        actions = []
        if_statement = self.if_statement()  # !! NO S, IF STATEMENTS AND IF STATEMENT IS DIFFERENT !!
    ...
    def while_statement(self, loopN=1):
        ...
        if self.token.value == "DO":
            ...
            while self.token.value != "END":
                ...
                if self.token.type == "DECL" or self.token.type == "RSV":
                    pass
                else:
                    self.move()
                n += 1
    ...
    def statement(self, loopN=1):
        ...
        elif self.token.type == "CC":
            return [self.token.type, self.token()]
        else:
            return self.boolean_expression()
